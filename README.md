...
Interaktives Marktradar-Dashboard für 45 über eToro oder Yuh handelbare Wertschriften in sechs Zukunftstechnologie-Feldern: BCI/Neurotech, KI-Agenten/ASI, humanoide Robotik, Quantencomputing, Biotech/Pharma und Krypto/CBDC.
...
- `portfolio-public.html` / `api/public-portfolio.js` (v36) — öffentlich teilbare, rein lesende Performance-Ansicht des Portfolios: keine "Neue Investition erfassen"/Bearbeiten/Löschen-UI, und die effektiv investierten USD-Beträge verlassen den Server nie (die GET-Funktion liefert je Position nur den relativen Portfolio-Anteil in % sowie Kurse/Performance/Retrospektive-Felder aus). Die Seite bietet zusätzlich einen Rechner: ein frei wählbares, virtuelles Vermögen wird proportional zur echten Gewichtung auf die Positionen verteilt und live in hypothetischen Wert sowie Gewinn/Verlust umgerechnet.
